...
const PickUpScreen = () => {
    ...
    const proceedToCart = () => {
        ...
        if(selectedDate && selectedTime && delivery) { navigation.replace("Cart", {
    ...
    return (
        ...
        <SafeAreaView style={styles.AndroidSafeArea}>
            ...
            <HorizontalDatepicker
                ...
                initialSelectedDate={new Date('2020-02-08')}
            ...
            <ScrollView horizontal showsVerticalScrollIndicator={false}>
                {times.map((item, index) => (
                    <Pressable key={index} onPress={() => setSelectedTime(item.time)} style={ selectedTime.includes(item.time) ? {margin:10,borderRadius:7,padding:15,borderColor:"red", borderWidth:0.7} : {margin:10,borderRadius:7,padding:15,borderColor:"gray", borderWidth:0.7}}>
                        <Text>{item.time}</Text>
                    </Pressable>
                ))}
            </ScrollView>
            ...
            <ScrollView horizontal showsVerticalScrollIndicator={false}>
                {deliveryTime.map((item, i) => (
                    <Pressable key={i} onPress={() => setDelivery(item.name)} style={ delivery.includes(item.name) ? {margin:10,borderRadius:7,padding:15,borderColor:"red", borderWidth:0.7} : {margin:10,borderRadius:7,padding:15,borderColor:"gray", borderWidth:0.7}}>
                        <Text>{item.name}</Text>
                    </Pressable>
                ))}
            </ScrollView>
